...
document.addEventListener("DOMContentLoaded", () => {
    ...
    .then(dados =>{
        console.log(dados);
        dados.forEach(dado => {
            chats.push(new chat(dado.id, dado.name, '/img/newGroupImg.svg', [user], [user],false));
        })
    })
    .catch(error => console.error('Erro ao enviar mensagem helloUser:', error));
});
class Obj {
...
class chat {
    constructor(id, name, thumb, guests, adm, gen = false) {
        ...
        this.msgs = [];
        if (this.gen) {
            const dataToSend = {id:this.id, name: this.name };
            fetch('/salvar-dados', {
                method: 'POST',
                headers: {
                'Content-Type': 'application/json',
    ...
    guestListFunction() {
        ...
        this.guests.forEach(guest => {
            ...
            const toAdm = new Obj('p', ['tornarAdm', 'material-symbols-outlined'], guestInfoMenu);
...
document.getElementById('createChatBtn').onclick = () => {
    if (nameInput.value.replace(/^\W+/, '') != '' && nameInput.value.length < 16) {
        chats.push(new chat(chats.length + 1, nameInput.value, '/img/newGroupImg.svg', [user], [user],true));
