...
class chat {
    ...
    createMsg(){
        ...
        this.msgBalloon.addEventListener('keydown',(event)=>{
            if(event.key === "Enter") {
                event.preventDefault();
                const newMsg = new msg(this.msgBalloon.value,null,new Date().getHours().toString().padStart(2, '0')+":"+new Date().getMinutes().toString().padStart(2, '0'),Zeulo,this);
                this.msgs.push(newMsg);
                this.msgBalloon.value = '';
            }
        })
        this.chatElement.appendChild(this.msgBalloon);
        const newMsg = new msg('this is a very long long message just for a few tests in my web page',null,'19:11',Lucas,this);
    }
}
class msg {
    ...
    createMsg(){
        ...
        this.msg.classList.add('msg');
            ...
            this.msgTextContent.innerText = this.content;
