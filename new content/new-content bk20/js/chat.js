...
let ldpalavrões = {
    A: {
      ...
      E:{
        synonyms: ["E","e","£","3","Ɛ","∃","∄","∈","∋","Ě","Ĕ","&","⋻","⋸","⋵","⋲","⋳","⋶","⋹","⋿","Ε","ε","Ἓ","Ἕ","ὲ","έ","Έ","έ","Σ","϶","ϵ","ξ","₠","€"],
        value: "E"
      },
      F:{
...
function intext() {
    ...
    for (let i = 0; i < textarea.value.length; i++) {
        palavra = textarea.value.charAt(i)
        let encontrado = false;
        for (let chave in ldpalavrões) {
            if (ldpalavrões[chave].synonyms.includes(palavra)) {
                console.log(palavra);
                encontrado = true;
                pd.push(ldpalavrões[chave].value);
                console.log(pd)
                break;
            }
        }
