...
function send() {
    ...
    if (textarea.value !== "" && textenvspace !== "") {
        ...
        } else if(textarea.value =="page://createGroup()") {
...
function imgMenu(event, element, imgPrev) {
    event.preventDefault()
    ...
    //incompleto (no lugar errado)
    element.firstElementChild.style.imageRendering = imgPrev.style.imageRendering
    //
...
textarea.addEventListener("drop", function (e) {
    ...
    for (let i = 0; i < e.dataTransfer.files.length; i++) {
        ...
        textarea.style.display = "none";
            if (file.type.startsWith('image/')) {
                ...
                textarea.value = "//" + `<a onclick='imgOpen(this)' oncontextmenu='imgMenu(event, this, imgPreview)' style='user-select:none;cursor:pointer'>${imgElement.outerHTML}</a>`;
    ...
    function previewImage(file) {
        /*const textareawh = document.getElementById('inhtml');
        const msg = document.getElementById('msg');
        msg.style.gridTemplateRows = '0fr 1fr';
        msg.style.padding= '0';
        textareawh.style.display = 'grid';
        const balloon = document.getElementById('balloon');
        balloon.style.gridTemplateColumns = '1fr 40px';
        balloon.style.gridTemplateRows = "80% 20%";
        textareawh.style.gridArea = '2 / 1 / 3 / 2';
        textareawh.style.background = '#99501b';
        const msgenv = document.getElementById('msg-env');
        msgenv.style.gridArea = '2 / 2 / 3 / 3';
        document.getElementById("ur-msg").addEventListener('input', function() {
            textareawh.innerHTML = document.getElementById("ur-msg").value; 
        })
        textareawh.addEventListener('input', function() {
            textarea.value = document.getElementById("inhtml").innerHTML;
        })
        ...
        */
        ...
        reader.onload = function (e) {
            ...
            //imgPreview.style.gridArea = '1 / 1 / 2 / 3'
            imgPreview.style.gridArea = '1 / 1 / 3 / 3'
        ...
        reader.onload = function(e) {
            ...
            if(textarea.value == '') {
                ...
                if(textarea.value.substring(0,2) == '//') {
                    textarea.value = textareaValue + `<div style='background: #152423;position: relative;padding: 5px;border-radius: 0.5rem;'><a title='download' style='color:white;background: #223937;padding: 0 5px;border-radius: 0.3rem;' href='${URL.createObjectURL(file)}' download="${file.name}">${file.name}<\a><br><a>"${content}"<\a><\div>`;
                } else {
                    textarea.value = '//'+ textareaValue + `<div style='background: #152423;position: relative;padding: 5px;border-radius: 0.5rem;'><a title='download' style='color:white;background: #223937;padding: 0 5px;border-radius: 0.3rem;' href='${URL.createObjectURL(file)}' download="${file.name}">${file.name}<\a><br><a>"${content}"<\a><\div>`;
                }
            }
    ...
    function previewHTMLFile(file) {
        ...
        reader.onload = function(e) {
            ...
            */
            ...
            textarea.value = `//<a href="${url}" target="_blank" style='color:white' title='acessar: ${file.name}'>${file.name}</a>`;
        };
        reader.readAsText(file);
        textarea.style.display = 'block';
      }
