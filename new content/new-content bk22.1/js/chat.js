...
textarea.addEventListener("drop", function (e) {
    ...
    for (let i = 0; i < e.dataTransfer.files.length; i++) {
        ...
        textarea.style.display = "none";
            if (file.type.startsWith('image/')) {
                ...
                previewVideo(file)
            } else if (file.type.startsWith('text/') || file.name.slice(-3) == '.md' || file.name.slice(-4) == '.jem' && file.type !== "text/html") {
    ...
    function previewTextFile(file) {
        ...
        reader.onload = function(e) {
            ...
            textarea.value = `//<a style='color:white' href='${URL.createObjectURL(file)}' download="${file.name}">${file.name}<\a> - "${content}"`;
        };
        reader.readAsText(file);
        textarea.style.display = "block";
    }
    function previewHTMLFile(file) {
        ...
        reader.onload = function(e) {
            ...
            previewLink.textContent = 'Visualizar HTML';
